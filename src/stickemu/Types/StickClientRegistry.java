   ...
   public void deregisterClient(StickClient client)
    {
        if (client.getLobbyStatus() && this.isLobby) {
            ...
            client.getRoom().BroadcastToRoom(StickPacketMaker.Disconnected(client.getUID()));
            // only allow access if another thread isn't reading the resource
            client.getRoom().GetCR().ClientsLock.writeLock().lock();
            try {
                client.getRoom().GetCR().Clients.remove(client.getUID());
            } finally {
                client.getRoom().GetCR().ClientsLock.writeLock().unlock();
            }
